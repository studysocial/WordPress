		...
		$('.wp_s3-settings').each(function() {
			...
			$('select.bucket', $container).change(function() {
				...
				var error_func = function(jqXHR, textStatus, errorThrown) {
					alert(wp_s3_i18n.create_bucket_error + errorThrown);
				...
				var success_func = function(data, textStatus, jqXHR) {
					if (typeof data['success'] !== 'undefined') {
						...
						wp_s3_i18n.create_bucket_nonce = data['_nonce'];
					}
					else {
						alert(wp_s3_i18n.create_bucket_error + data['error']);
						$select[0].selectedIndex = 0;
					}
				};

				var bucket_name = window.prompt(wp_s3_i18n.create_bucket_prompt);
				if (!bucket_name) {
					$select[0].selectedIndex = 0;
					return;
				}
				...
				var data = {
					action: 		'wp_s3-create-bucket',
					bucket_name: 	bucket_name,
					_nonce:			wp_s3_i18n.create_bucket_nonce
				};
